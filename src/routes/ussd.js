...
router.post('/', async (req, res) => {
  ...
  if (!sessions[sessionId]) {
    sessions[sessionId] = { step: 'menu', data: {} };
  }
  ...
  try {
    ...
    switch (session.step) {
      case 'menu':
        ...
        break;
      case 'register_name':
        response = await handleRegister(session, phoneNumber, lastInput);
        break;
      case 'new_tx_seller':
...
async function handleNewTxSeller(session, phone) {
  ...
  session.data.sellerPhone = phone;
  session.step = 'new_tx_amount';
  return ussdResponse(`Seller: ${seller.name} (${phone})\nEnter amount in GHS:`);
}
